...
function liftCalcPivotToFunc(ex::Expr, by::Array{Symbol,1})
    ...
    if !isempty(by) # micro split-apply-combine
            ...
            push!(
                ...
                Expr(
                    ...
                    Expr(
                        :call,
                        :(Base.invokelatest),
                        Expr(:ref, :aggrfuncs, QuoteNode(a)),
                        lambdasym,
                    ),
                ),
            )
        ...
        code = :(function $funnameouter(_df_::AbstractDataFrame, c::Symbol; kwargs...) # we need kwargs here for aggregate specs
            aggrfuncs = Dict{Symbol,Function}()
            for (aggrc, spec) in kwargs
                # this would throw if spec is not compliant, as usual
                aggrfuncs[aggrc] = $(@__MODULE__).liftAggrSpecToFunc(aggrc, spec)
            end
        ...
        funargs = map(x -> Expr(:ref, :_df_, :!, QuoteNode(x)), collect(keys(membernames)))
        code = :(function $funnameouter(_df_::AbstractDataFrame) # we need kwargs here for aggregate specs
            function $funname($(collect(values(membernames))...))
                $cex
            end
            $funname($(funargs...))
